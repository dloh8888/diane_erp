...
function parseKeywordTrend(rows) {
  return rows.map((r) => ({
    period: String(byExactHeader(r, '기간구분') || '').trim(),
    base_date: toDateString(byExactHeader(r, '기준일')) || null,
    platform: String(byExactHeader(r, '플랫폼') || '').trim(),
    country: String(byExactHeader(r, '국가') || '').trim(),
    category: String(byExactHeader(r, '카테고리') || '').trim(),
    product_name: String(byExactHeader(r, '상품명') || '').trim(),
    brand: String(byExactHeader(r, '브랜드') || '').trim(),
    sold: num(byExactHeader(r, '누적판매량')),
    asp: num(byExactHeader(r, 'ASP')),
  })).filter((r) => r.period && r.base_date && r.platform && r.product_name);
}

function parseSearchTrend(rows) {
  return rows.map((r) => ({
    collected_at: toDateString(byExactHeader(r, '수집일')) || null,
    country: String(byExactHeader(r, '국가') || '').trim(),
    category: String(byExactHeader(r, '카테고리') || '').trim(),
    type: String(byExactHeader(r, '유형') || '').trim(),
    grp: String(byExactHeader(r, '그룹') || '').trim(),
    keyword: String(byExactHeader(r, '키워드') || '').trim(),
    week: toDateString(byExactHeader(r, '주차')) || null,
    value: num(byExactHeader(r, '지수')),
  })).filter((r) => r.keyword && r.week);
}

function parseRising(rows) {
  return rows.map((r) => ({
    collected_at: toDateString(byExactHeader(r, '수집일')) || null,
    country: String(byExactHeader(r, '국가') || '').trim(),
    category: String(byExactHeader(r, '카테고리') || '').trim(),
    query: String(byExactHeader(r, '급상승 검색어') || '').trim(),
    growth: String(byExactHeader(r, '증가율') || '').trim(),
    note: String(byExactHeader(r, '메모') || '').trim(),
  })).filter((r) => r.query);
}
...
export const SHEET_PLAN = [
  ...
  { sheet: '키워드트렌드', table: 'keyword_trend', parse: parseKeywordTrend, required: false },
  { sheet: '검색트렌드', table: 'search_trend', parse: parseSearchTrend, required: false },
  { sheet: '급상승', table: 'rising_search', parse: parseRising, required: false },
];
