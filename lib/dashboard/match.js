...
import {
  ...
  K_BRAND_VALUES, K_CATEGORY_VALUES, ITEM_CATEGORIES, COUNTRY_ALIASES,
...
/**
 * 국가 표기를 두 글자 코드로 맞춥니다.
 * '말레이시아(서)' → 'MY', '필리핀' → 'PH', 'ph' → 'PH'.
 * 목록에 없는 표기는 적힌 그대로 두어서, 새 국가가 생겨도 숫자가 사라지지 않게 합니다.
 */
export function normCountry(v) {
  ...
  for (const code of Object.keys(COUNTRY_ALIASES)) {
    for (const alias of COUNTRY_ALIASES[code]) {
      ...
      if (key === a) return code;
    }
  }
  // '말레이시아(서)' 처럼 괄호가 붙은 경우 괄호를 떼고 한 번 더
  const bare = key.replace(/\([^)]*\)/g, '');
  if (bare && bare !== key) {
    for (const code of Object.keys(COUNTRY_ALIASES)) {
      for (const alias of COUNTRY_ALIASES[code]) {
        if (bare === alias.toLowerCase().replace(/[\s\-_.]/g, '')) return code;
      }
    }
  }
  return raw;
...
/**
 * 위 matchKBrandEntry 와 판정 결과는 같지만 훨씬 빠른 버전입니다.
 *
 * K브랜드목록이 6천 개가 넘으면 줄마다 목록을 전부 훑는 방식은 너무 느립니다
 * (2만 줄 × 6천 개 = 1억 번 넘는 비교). 그래서 목록을 미리 사전(Set)으로 만들어 두고
 * 한 번에 찾습니다. 판정 규칙(통째로 같음 / 조각이 같음 / 앞말로 시작) 은 그대로입니다.
 *
 * 다른 점 하나: 앞말이 여러 개 걸릴 때 목록 순서가 아니라 '가장 긴 것' 을 고릅니다.
 * (더 정확한 쪽입니다. K브랜드인지 아닌지의 결과는 똑같고, 화면에 표시되는
 *  '어느 항목에 맞았는지' 만 달라질 수 있습니다)
 */
export function createKBrandMatcher(list) {
  const exact = new Set();
  const prefixMinLen = new Map(); // 앞말 매칭을 허용하는 항목 -> 최소 길이

  for (const item of list || []) {
    const target = item && item.n !== undefined ? item.n : item;
    if (!target) continue;
    exact.add(target);
    const minLen = item && item.p ? 2 : 3;
    if (target.length >= minLen) {
      const prev = prefixMinLen.get(target);
      if (prev === undefined || minLen < prev) prefixMinLen.set(target, minLen);
    }
  }

  return function match(brandName) {
    const raw = String(brandName == null ? '' : brandName);
    const key = normBrand(raw);
    if (!key || !exact.size) return '';

    if (exact.has(key)) return key; // 통째로 같음
    ...
    for (let len = key.length - 1; len >= 2; len--) {
      ...
      const minLen = prefixMinLen.get(p);
      if (minLen !== undefined && p.length >= minLen) return p;
    }
