...
function RankDelta({ delta }) {
  const { t } = useLang();
  if (delta === null || delta === undefined) return <span className="text-gray-400 text-xs">{t('신규')}</span>;
      ...
      >
        ...
        <div className="flex flex-wrap items-end gap-3 mb-3.5">
          <div className="flex flex-col gap-1 text-xs text-gray-400">
            ...
            >
              <option value="brand">{t('브랜드')}</option>
              <option value="mid">{t('카테고리 대분류')}</option>
            </select>
          </div>
          ...
          <div className="text-xs text-gray-400 ml-auto pb-1.5">
            ...
            {source.total > source.shown && (
              <span className="text-gray-300"> {t('· 원본')} {source.total.toLocaleString('ko-KR')}{t('개 중 상위')} {source.shown}{t('개만 계산')}</span>
            )}
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-xs min-w-[900px]">
            <thead>
              <tr>
                <th className="py-2 px-2 text-center font-medium text-gray-400 text-[11px] align-bottom">{t('순위')}</th>
                <th className="py-2 px-2 text-left font-medium text-gray-400 text-[11px] align-bottom">
                  {kind === 'brand' ? t('브랜드') : t('카테고리 대분류')}
                </th>
                <th className="py-2 px-2 text-center font-medium text-gray-400 text-[11px] align-bottom">{t('변동')}</th>
                <th className="py-2 px-2 text-center font-medium text-gray-400 text-[11px] align-bottom">K</th>
                <Th label={t('일평균 GMV')} k="dailyGmv" />
                <Th label={t('BAU 일평균')} k="bauDailyGmv" />
                <Th label={t('증감율')} k="growthPct" />
                <Th label={t('비중')} k="sharePct" />
                {kind === 'brand' && <Th label={t('대분류 내 비중')} sub={t('(BAU 대비 %p)')} k="parentSharePct" />}
                <Th label={t('총 GMV')} k="gmv" />
                <Th label={t('주문')} k="orders" />
                <Th label="AOV" k="aov" />
                <Th label={t('증감율')} k="aovGrowthPct" />
              </tr>
            </thead>
            <tbody>
              {rows.slice(0, limit).map((r) => (
                <tr key={r.name}>
                  ...
                  <td className="py-2 px-2 text-left border-b border-gray-100">
                    <span className="block">{t(r.name)}</span>
                    {r.nameEn && <span className="block text-[11px] text-gray-400">{r.nameEn}</span>}
                    {kind === 'brand' && r.parent && (
                      <span className="block text-[10px] text-gray-300">{t(r.parent)}</span>
                    )}
                  </td>
